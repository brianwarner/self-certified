# CLI launcher for Internal Certificate Manager (PowerShell)
...
Write-Host ""
Write-Status "Internal Certificate Manager - CLI"
Write-Host ""
...
try {
    & python -c "import cryptography, prompt_toolkit" 2>$null
    if ($LASTEXITCODE -ne 0) {
        $NeedsInstall = $true
    }
} catch {
...
if ($NeedsInstall) {
    ...
    $Outdated = & pip list --outdated --format=columns 2>$null | Select-String -Pattern "(cryptography|prompt-toolkit)"
    if ($Outdated) {
